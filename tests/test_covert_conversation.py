...
from src.token_stego.conversation import CovertConversation, text_transport
...
def _make_mock_model(vocab_size: int = VOCAB_SIZE) -> MagicMock:
    ...
    model.detokenize = lambda ids: mock_decode(ids, skip_special_tokens=False)
...
class TestCovertConversationRoundTrip:
    ...
    def test_noninvertible_text_transport_fails_closed(self) -> None:
        model = _make_mock_model()
        model.detokenize = lambda _ids: "lossy text"
        model.tokenize = lambda _text: [1, 2, 3]
        conv = CovertConversation(model=model, cover_topic="Discuss computing")
        ...
        try:
            ...
            assert "text transport" in str(exc)
        else:
            raise AssertionError("Lossy token transport must not report a channel")

    def test_real_fast_tokenizer_fixture_preserves_declared_ids(self) -> None:
        ...
        model = StegoModel("local-tokenizer-fixture")
        model._tokenizer = tokenizer

        assert text_transport(model, [1, 2]) == "hello world"
