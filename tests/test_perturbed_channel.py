...
class TestSaveLoadPerturbation:
    ...
    def test_invalid_checkpoint_schema_is_rejected(self) -> None:
        with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as file_handle:
...
def test_perturbed_keyed_decode_requires_nonce_before_model_access() -> None:
    mock_model = _make_mock_model()
    inner = ArithmeticStegoChannel(model=mock_model, key=b"test-key")
    channel = PerturbedStegoChannel(
        inner, DistributionPerturbation(HIDDEN_DIM, VOCAB_SIZE)
    )

    with pytest.raises(ValueError, match="nonce"):
        channel.decode("TOKENS:1", "prompt", num_bits=1)
    mock_model.tokenize.assert_not_called()
