...
BITS_PER_ASCII_CHARACTER = 8


@dataclass
class ChannelMetrics:
    """Payload-level channel measurements.

    ``total_bits`` counts recovered payload bits, excluding framing overhead.
    Character decoders use the repository's one-byte ASCII payload contract;
    bit decoders count the compared payload bits directly. ``bits_per_second``
    is ``total_bits / elapsed_seconds`` when elapsed time is positive.
    """
...
def ascii_payload_bits(character_count: int) -> int:
    """Convert a count of characters under the ASCII contract to payload bits."""
    if character_count < 0:
        raise ValueError("character_count must be non-negative")
    return character_count * BITS_PER_ASCII_CHARACTER
