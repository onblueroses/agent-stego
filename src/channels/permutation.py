...
class PermutationChannel(BaseChannel):
    ...
    def __init__(self, items: list[str] | None = None):
        self.items = list(DEFAULT_ITEMS) if items is None else list(items)
        if len(set(self.items)) != len(self.items):
            raise ValueError("Permutation items must be unique")
        self.n = len(self.items)
        self.capacity = math.factorial(self.n)
        if self.capacity < 26:
            raise ValueError(
                "Permutation alphabet requires at least 26 distinct orderings"
            )
