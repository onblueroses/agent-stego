"""Table-based covert-channel mechanics using lookup tables.

A fixed table maps each letter to a tool call. The model-facing prompt asks for
exact lookup and reproduction rather than counting or argument-length control.
Historical model-performance measurements are not part of this module's
contract; see the qualified reports under ``analysis/``.
...
class TableChannel(BaseChannel):
    ...
    def measure(self, original: str, traces: list[ToolTrace]) -> ChannelMetrics:
        ...
        bits_total = ascii_payload_bits(compare_len)
