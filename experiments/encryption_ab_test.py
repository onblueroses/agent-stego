"""Descriptive detector comparison with and without SHAKE-256 masking.

This driver reports observations for four frozen replay cells. It does not turn
them into a detection or evasion verdict without a powered design and a same-rig
positive control.
...
def run_condition(
    ...
    for i in range(n_samples):
        prompt = PROMPTS[i]
        text = generate_normal_text(model, prompt, max_tokens, rng, temperature, top_p)
...
def run_ab_test() -> None:
    ...
    print("SHAKE-256 Masking Comparison")
    ...
    # C: Masking + the frozen distribution-coupling cell
    ...
    lowest_name, lowest_result = min(
        conditions, key=lambda x: x[1]["auc_entropy_oriented"]
    )
    print(
        f"\nLowest observed entropy AUC: {lowest_name} "
        f"({lowest_result['auc_entropy_oriented']:.3f})"
    )
    print(
    ...
    print(
        "UNTRIED: other coupling values, model families, task families, payload "
        "lengths, and data regimes."
    )
    print(
        "NEXT ROUTE: preregister a detectable effect and positive control, then "
        "size independent prompt groups before rerunning."
    )
